...
var
  ...
  MNames: TRTTINames;
begin
  MNames := g3rtti.GetClassMethodNames(Self, False);
  ...
  begin
    ...
    //Result[i].Command    :=
    //g3log.Info('Found test: ' + MNames[i]);
  end;
end;
...
begin
  ...
  g3log.Info('Test result statistics:');
  g3log.Info('  Not run:   ' + IntToStr(Stats[trNone]));
  g3log.Info('  Disabled:  ' + IntToStr(Stats[trDisabled]));
  g3log.Info('  Passed:    ' + IntToStr(Stats[trSuccess]));
  g3log.Info('  Failed:    ' + IntToStr(Stats[trFail]));
  g3log.Info('  Exception: ' + IntToStr(Stats[trException]));
  g3log.Info('  Error:     ' + IntToStr(Stats[trError]));
end;

procedure TLogTestRunner.HandleCreateSuite(Suite: TTestSuite);
begin
  g3log.Info('Created suite instance of class "' + Suite.ClassName + '"');
end;

procedure TLogTestRunner.HandleDestroySuite(Suite: TTestSuite);
begin
  g3log.Info('Destroyed suite instance of class "' + Suite.ClassName + '"');
end;
...
begin
  ...
  case TestResult of
    trNone: g3log.Info('  not run');
    trDisabled: g3log.Info('  disabled');
    trSuccess: g3log.Info('  passed');
    trFail: g3log.Warning('  failed' + CodeLocToStr(ATest.FailCodeLoc));
    trException: g3log.Fatal('  exception');
    trError: g3log.Error('  error');
  end;
end;
...
begin
  if Assigned(E) then
    g3log.Error('Exception in test "' + String(ATest.Name) + '" with message: ' + E.Message);
end;

function TLogTestRunner.IsTestEnabled(const ATest: TTest): Boolean;
begin
  Result := True;
  g3log.Info('Test: "' + string(ATest.Name) + '"...');
end;
